...
describe("OpenAPI Quickstart Scenario", () => {
  ...
  it("runs 'npm run dev' and expects successful execution with default openapi.yaml", () => {
    ...
    assert.strictEqual(result.status, 0, `npm run dev should exit with code 0, got ${result.status}`);
  ...
  it("runs 'npm run dev' with a custom file argument and expects successful execution", () => {
    ...
    fs.writeFileSync(customFile, "openapi: 3.1.0\ninfo:\n  title: Custom API\n  version: 1.0.0\npaths: {}\n");
    // Run npm script with custom file
    const result = spawnSync('npm', ['run', 'dev', '--', customFile], { encoding: 'utf-8' });
    assert.strictEqual(result.status, 0, `npm run dev -- ${customFile} should exit with code 0, got ${result.status}`);
    assert.match(result.stdout, /custom-openapi.yaml/i, "Output should mention custom-openapi.yaml");
    // Clean up
